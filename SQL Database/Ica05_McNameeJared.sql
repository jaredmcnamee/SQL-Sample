...
select 
TrackID as 'Track ID', 
convert(varchar(50),Name) as 'Title', 
Composer
from Track
where 
(Name Like 'Black%' 
...
'Track ID', 
convert(time(3),
DATEADD(millisecond,Milliseconds,0),114) as Time, 
cast (convert(float, UnitPrice) *60000 / Milliseconds as money) as 'Cost/Minute',
CAST(cast(Bytes as decimal)/cast(Milliseconds as decimal)*1000.0 as decimal(10,3)) as 'Bytes/Second'
from Track
where
Milliseconds > 60000
and cast (convert(float, UnitPrice) *60000 / Milliseconds as money) > 2.75
